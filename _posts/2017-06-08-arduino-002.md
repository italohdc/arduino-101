...
Ao criar um código, as vezes você quer que um procedimento seja executado somente se uma condição qualquer seja satisfeita. Para fazer isso, pode ser utilizado o comando ``if``. O comando ``if`` é um comando que irá realizar o conteúdo dentro das chaves ``{}`` somente se a condição que estiver dentro dele for satisfeita.
